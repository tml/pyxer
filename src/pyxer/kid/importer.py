...
from pyxer.kid import __version__
from pyxer.kid.codewriter import raise_template_error
from pyxer.kid.compiler import KidFile, KID_EXT
from pyxer.kid.template_util import TemplateImportError
...
def get_template_name(name, filename):
    if name:
        ...
        return 'pyxer.kid.util.template_%x' % (hash(filename) + sys.maxint + 1)
