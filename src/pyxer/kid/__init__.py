...
from pyxer.kid.util import xml_sniff, QuickTextReader
from pyxer.kid.namespace import Namespace
from pyxer.kid.codewriter import KID_XMLNS, raise_template_error
from pyxer.kid.compiler import KID_EXT
from pyxer.kid.element import Element, SubElement, Comment, \
    ProcessingInstruction, Fragment
from pyxer.kid.parser import ElementStream, XML, document, _coalesce
from pyxer.kid.filter import transform_filter
from pyxer.kid.serialization import Serializer, PlainSerializer, \
    XMLSerializer, HTMLSerializer, XHTMLSerializer
from pyxer.kid.format import Format, output_formats
from pyxer.kid import template_util
...
def enable_import(ext=None, path=None):
    ...
    """
    import pyxer.kid.importer
    pyxer.kid.importer.install(ext, path)


def disable_import(path=None):
    """Disable the kid module loader and import hooks again."""
    import pyxer.kid.importer
    pyxer.kid.importer.uninstall(path)
...
def import_template(name, encoding=None):
    ...
    This function can be used to achieve the same result as follows::
        ...
        import pyxer.kid
        foo = pyxer.kid.import_template('foo')
...
def load_template(file, name='', cache=True, encoding=None, ns={},
    ...
    if isinstance(file, basestring):
        ...
        fo = file
        filename = '<string>'
    import pyxer.kid.importer as importer
    if filename != '<string>':
