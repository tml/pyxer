...
def test_strip_no_expr():
    ...
    """
    data = pyxer.kid.Template(source=source).serialize()
...
def test_strip_with_boolean_expression__or():
    ...
    """
    t = pyxer.kid.Template(source=source_template % (True, True))
    assert "<el>" not in t.serialize()
    t = pyxer.kid.Template(source=source_template % (True, False))
    assert "<el>" not in t.serialize()
    t = pyxer.kid.Template(source=source_template % (False, True))
    assert "<el>" not in t.serialize()
    t = pyxer.kid.Template(source=source_template % (False, False))
